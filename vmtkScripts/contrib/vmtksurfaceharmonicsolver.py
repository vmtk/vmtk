...
class vmtkSurfaceHarmonicSolver(pypes.pypeScript):
    ...
    def __init__(self):
        ...
        self.SetInputMembers([
            ...
            ['VectorialEq','vectorialeq','bool',1,'','toggle solving the vectorial Laplace-Beltrami equation instead of the scalar one'],
            ['SolutionArrayName', 'solutionarray', 'str', 1, '','name of the point-data array where the solution is stored'],
            ['InputRings','irings','vtkPolyData',1,'','the optional input rings, with BC values possibly stored in InputRingsBCsArrayName; this vtkPolyData can contain more than one ring','vmtksurfacereader'],
            ['InputRingsBCsArrayName', 'ringsbcsarray', 'str', 1, '','name of the point-data array where the BCs values are stored; if None, the value zero is taken'],
            ['ExcludeIds','excludeids','int',-1,'','entity ids excluded from the equation domain'],
            ...
            ['InitWithZeroDirBCs','zerodirbcs','bool',1,'','set homogeneous Dirichlet conditions on all boundaries that are not set interactively or via InputRings'],
