...
## Program:   VMTK
## Module:    $RCSfile: vmtkmeshringgenerator.py,v $
## Language:  Python
...
class vmtkMeshRingGenerator(pypes.pypeScript):
    ...
    def __init__(self):
        ...
        self.SetInputMembers([
            ...
            ['UseBoundaryNormal','useboundarynormal','bool',1,'','toggle using average boundary normal as extrusion direction, instead of local normals'],
    ...
    def Execute(self):
        ...
        if self.UseBoundaryNormal:
            normals = ring.GetPointData().GetArray('Normals')
            nx = ny = nz = 0.0
            for i in range(normals.GetNumberOfTuples()):
                nx = nx + normals.GetComponent(i,0)
                ny = ny + normals.GetComponent(i,1)
                nz = nz + normals.GetComponent(i,2)

            nx = nx / normals.GetNumberOfTuples();
            ny = ny / normals.GetNumberOfTuples();
            nz = nz / normals.GetNumberOfTuples();

            normals.FillComponent(0,nx)
            normals.FillComponent(1,ny)
            normals.FillComponent(2,nz)
